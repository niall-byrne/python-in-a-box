...
class TestPostGenGitSetup(TestCase):
    ...
    @patch.dict(os.environ, {}, clear=True)
    def test_condition_false_another_format(self) -> None:
        with patch(post_gen_project.__name__ + ".Template") as m_template:
    ...
    @patch.dict(os.environ, {"PIB_SKIP_FMT_INIT": "1"}, clear=True)
    ...
    @patch("os.walk")
    @patch("os.getcwd")
    def test_find_python_files(self, m_cwd: Mock, m_walk: Mock) -> None:
    ...
    @patch("os.system")
    @patch("importlib.import_module")
    def test_format_yapf_not_found(
        self,
        m_import: Mock,
        m_system: Mock,
    ) -> None:
